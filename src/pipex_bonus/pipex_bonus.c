...
/*   Updated: 2025/02/13 12:27:04 by mgouraud         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	cmd_exec(t_pipex **data, int cmd_index, int step, char *envp[]);
...
void	cmd_forking(t_pipex **data, int cmd_index, int step, char *envp[])
{
	...
	else
	{
		...
		if (pid < 0)
			...
			cmd_exec(data, cmd_index, step, envp);
	}
}
...
static void	cmd_exec(t_pipex **data, int cmd_index, int step, char *envp[])
{
	...
	else if (dup2((*data)->pipes[cmd_index + 1][1], STDOUT_FILENO)
			== -1)
		error_handler(ERR_DUP, data, 1);
